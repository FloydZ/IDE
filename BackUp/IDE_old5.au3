...
Global $CurrentLanguage = IniRead($MAINOPTFILE, "CurrentLanguage", "Current", 0);Pointer auf die Sprache im LanguageArray
...
Next
GUICtrlSetState($LanguageItemArray[$CurrentLanguage], $GUI_CHECKED)
...
Func _ChangeLanguage()
	For $x = 0 to UBound($LanguageItemArray) -1
		If @GUI_CtrlId = $LanguageItemArray[$x] Then
			GUICtrlSetState($LanguageItemArray[$CurrentLanguage], $GUI_UNCHECKED)
			GUICtrlSetState($LanguageItemArray[$x], $GUI_CHECKED)
			$CurrentLanguage = $x
			IniWrite($MAINOPTFILE, "CurrentLanguage", "Current", $CurrentLanguage)
		EndIf
	Next
EndFunc
;=============================Compile==================================================
...
Func _CompileAutoitFile();Nichtgut
	$File = $FileArray[$TabArray[_GUICtrlTab_GetCurSel($hTab)][2]][2]

	$Compiler = IniRead($MAINOPTFILE, "IDEOptionsDirections", "Bin", 0)
	$Compiler &= "\Aut2exe.exe";Muss noch
...
Func _OptionsIDEDirections()
	...
	$Sections = IniReadSection($MAINOPTFILE, "IDEOptionsDirections")
	For $x = 1 to $Sections[0][0]
		GUICtrlCreateListViewItem($Sections[$x][0] & "|" & $Sections[$x][1], $OptionsListview)
	Next
	...
	GUICtrlCreateLabel("Directions", 5, 45, 51, 17)
...
Func _OptionsSaveIDEOptions()
	...
	For $x = 0 to _GUICtrlListView_GetItemCount($Handle) - 1
		IniWrite($MAINOPTFILE, "IDEOptionsDirections", _GUICtrlListView_GetItemText($Handle, $x, 0),  _GUICtrlListView_GetItemText($Handle, $x, 1))
	Next
