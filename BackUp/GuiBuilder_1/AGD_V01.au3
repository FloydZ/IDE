...
$tbw = GUICreate("AGD ToolBox",150,300,(@DesktopWidth/2) -312,(@DesktopHeight / 2) - 166)
GUISetState (@SW_SHOW)
$mw = GUICreate("AutoIT GUI Designer",300,300,-1,-1,$WS_SIZEBOX,-1,$tbw)
GUISetState (@SW_SHOW)
GUISwitch($tbw)
for $i= 1 to $btnt[0]
	$btn[$i] = GUICtrlCreateButton($btnt[$i],5,5 + (22 *($i-1)),60,20)
next
$genbtn = GUICtrlCreateButton("Gen Code",5,255,60,20)
$savebtn = GUICtrlCreateButton("Save",75,255,60,20)
$loadbtn = GUICtrlCreateButton("Load",75,235,60,20)
...
While 1
	...
	for $cti = 1 to $btnt[0]
		if $msg = $btn[$cti] then
			...
			switch $btnt[$cti]
				case "Button"
					$tbtn[$cbi] = GUICtrlCreateButton($btnt[$cti],5,5,50,20)
				case "Input"
