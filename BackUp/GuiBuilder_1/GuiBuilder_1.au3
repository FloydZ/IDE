#Region converted Directives from C:\Users\omasgehstock\Dropbox\Engine_Release\IDE\BackUp\GuiBuilder_1\GuiBuilder_1.au3.ini
#AutoIt3Wrapper_aut2exe=D:\Programmi\AutoIt3\aut2exe\aut2exe.exe
#AutoIt3Wrapper_outfile=D:\Programmi\AutoIt3\Varie\GuiBuilder_1\GuiBuilder_1.exe
...
#EndRegion converted Directives from C:\Users\omasgehstock\Dropbox\Engine_Release\IDE\BackUp\GuiBuilder_1\GuiBuilder_1.au3.ini
;
...
WinMove("handle=" & $main,"", 290, @DesktopHeight/2-175, 400,350)  ;display size of window
...
#region - Roy add-on
$menu3 = GUICtrlCreateMenuitem("SaveDefinition", -1)
$menu4 = GUICtrlCreateMenuitem("LoadDefinition", -1)
Global $AgdOutFile = ""
...
#ce
   ...
   If $cursorInfo[0] <= 0 Or $cursorInfo[1] <= 0 Or $cursorInfo[0] >= $wgcs[0] Or $cursorInfo[1] >= $wgcs[1] Then
      ...
      ContinueLoop
   EndIf
   ;WinSetTitle("handle=" & $main, "", $cursorInfo[0] & ", "& $cursorInfo[1] & "  :  " & $wgcs[0] & " x " & $wgcs[1])
...
$code = "; Script generated by AutoBuilder 0.5 Prototype" & @CRLF & @CRLF & "#include <GuiConstants.au3>" & @CRLF & @CRLF
...
;   $client = WinGetClientSize("handle=" & $nullParent)
;   $window = WinGetPos("handle=" & $nullParent)
...
Local $n = 0, $Key, $Text
	...
	If $AgdOutFile = "" Then
		$AgdOutFile = FileSaveDialog("Save GUI Definition to file?", @MyDocumentsDir, "AutoIt Gui Definitions (*.agd)", 2+16)
		If @error Then
			SplashTextOn("Save GUI Definition to file", @CRLF & "Definition not saved!")
			Sleep(1000)
			SplashOff()
			Return 
		EndIf
	...
	IniWrite($AgdOutFile, "Main", "guiheigth",$h)
	...
	SplashTextOn("Save GUI Definition to file", @CRLF & "Saved to " & $AgdOutFile)
	Sleep(1000)
	SplashOff()
EndFunc
...
Func _LoadGuiDefinition()
	...
	$AgdInfile = FileOpenDialog("Load GUI Definition from file?", @MyDocumentsDir, "AutoIt Gui Definitions (*.agd)", 1)
...
; Clean current gui
	...
	$h = IniRead($AgdInfile, "Main", "guiheigth",-1)
...
; 21 Slider
    ...
    $grippyCtrl = 0
	_repaintWindow()
	SplashTextOn("Load GUI Definition from file", @CRLF & "Loaded from " & $AgdInfile)
	$AgdOutFile = ""
	Sleep(1000)
	SplashOff()
EndFunc
